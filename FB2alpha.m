function [ alpha, fitness1,fitness2 ] = FB2alpha( FBpairs, img, trimap, is_sort )
%FB2ALPHA£º Output FB as alpha.
%   Detailed explanation goes here
    ...
    %% Sort foreground and background points based on HSV.
    if is_sort
        if is_sort ==1
...
%         U_hsv = img_hsv(U_ind,:);
    ...
    Obj4 = sum(abs(U_rgb - (alphaX3.*Fx_rgb+(1-alphaX3).*Bx_rgb)),2);
    ...
    Obj5 = sum(abs(Bx_yx-U_yx),2)./B_mindist;
    Obj6 = sum(abs(Fx_yx-U_yx),2)./F_mindist;
    fitness1 = mean(sum([Obj1,Obj2,Obj3]'));
    fitness2 = mean(sum([Obj4,Obj5,Obj6]'));
end
